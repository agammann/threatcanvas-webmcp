...
const STORAGE_KEY = 'threatcanvas-workspace-v1';
...
export function ThreatCanvasApp() {
  ...
  const sprintView = <>
    ...
    {!sprint ? <section className="tc-empty-sprint"><span><Zap /></span><h3>Build a capacity-aware sprint</h3><p>ThreatCanvas will select the strongest risk-to-effort mix without resolving anything.</p><label>Engineering capacity <div><input type="number" min="0.5" max="60" step="0.5" value={sprintCapacity} onChange={(event) => setSprintCapacity(Number(event.target.value))} /><span>days</span></div></label><Button size="lg" onClick={createSuggestedSprint}><Sparkles />Create suggested sprint</Button></section> : <>
  ...
  const webmcpView = <>
    ...
    <section className="tc-state-diagram" aria-label="WebMCP shared-state diagram"><div><UserRound /><strong>Human analyst</strong><small>Reviews, edits, locks</small></div><ArrowRight /><div><LayoutDashboard /><strong>ThreatCanvas UI</strong><small>Visible source of truth</small></div><ArrowRight /><div className="focus"><Boxes /><strong>Shared application state</strong><small>One workspace, local persistence</small></div><ArrowRight /><div><Braces /><strong>WebMCP tools</strong><small>Structured application actions</small></div><ArrowRight /><div><Bot /><strong>AI agent</strong><small>Inspects, compares, plans</small></div></section>
    <section className="tc-without-with"><article><span className="tc-eyebrow">Without WebMCP</span><h3>Infer the interface</h3><p>An agent must inspect the page, locate controls, simulate clicks, and infer whether each step worked.</p></article><article><span className="tc-eyebrow">With WebMCP</span><h3>Use explicit capabilities</h3><p>ThreatCanvas exposes auditable operations tied directly to validated domain logic and visible state.</p></article></section>
